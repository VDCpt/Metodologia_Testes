...
async function exportPDF() {
    ...
    logAudit('📄 A gerar Parecer Técnico (Estilo Institucional v1.0-COMMERCIAL-LITIGATION)...', 'info');
	...
	// FIX-PAGEH-01: pageH declarado no scope principal de exportPDF().
	// pageHeight existia apenas dentro da closure addWatermark — inacessível aqui.
	// pageW já existe na linha ~7534 no scope da secção Risk Verdict — não redeclarar.
	const pageH = doc.internal.pageSize.getHeight(); // altura A4 em mm (297)
        // ────────────────────────────────────────────────────────────────────
        ...
        if (typeof window.generateLegalNarrative === 'function') {
            try {
                logAudit('🤖 [v1.0] A gerar Síntese Jurídica + Simulador Adversarial (IA)...', 'info');
                _enrichLegalNarrative = await window.generateLegalNarrative(UNIFEDSystem.analysis);
                logAudit('✅ [v1.0] Síntese Jurídica + Contra-Interrogatório gerados.', 'success');
            } catch (_aiErr) {
                _enrichLegalNarrative = '[Síntese jurídica indisponível — motor forense íntegro]';
                logAudit('⚠ [v1.0] IA indisponível — PDF gerado sem síntese (CORS/offline).', 'warning');
            }
        }

        if (typeof window.renderSankeyToImage === 'function') {
            try {
                logAudit('📊 [v1.0] A renderizar Diagrama de Fluxo Financeiro (Sankey)...', 'info');
                _enrichSankeyImage = await window.renderSankeyToImage(UNIFEDSystem.analysis);
                if (_enrichSankeyImage) {
                    logAudit('✅ [v1.0] Diagrama Sankey renderizado com sucesso.', 'success');
                } else {
                    logAudit('⚠ [v1.0] Diagrama Sankey indisponível — PDF gerado sem gráfico.', 'warning');
                }
            } catch (_sankeyErr) {
                _enrichSankeyImage = null;
                logAudit('⚠ [v1.0] Erro Sankey — PDF gerado sem diagrama.', 'warning');
            }
        }
        if (typeof window.generateTemporalChartImage === 'function') {
            try {
                logAudit('📅 [v1.0] A renderizar Gráfico ATF (Análise Temporal Forense)...', 'info');
                ...
                if (_enrichTemporalImage) {
                    logAudit('✅ [v1.0] Gráfico ATF renderizado com sucesso.', 'success');
                }
            } catch (_atfErr) {
                _enrichTemporalImage = null;
                logAudit('⚠ [v1.0] ATF gráfico indisponível — PDF sem análise temporal.', 'warning');
            }
        }
        ...
        // ── newPage: helper canónico — garante addFooter + addHeader + y=PDF_MARGIN_T
        // FIX-OVERLAP-01: substitui todos os padrões bare addPage()+y=20 que causavam
        // sobreposição de texto no header (rect y=7→22mm). Usar sempre esta função.
        const newPage = (doc) => {
            addFooter(doc, pageNumber);
            doc.addPage();
            pageNumber++;
            addHeader(doc);
            return PDF_MARGIN_T; // y inicial seguro (35mm, abaixo do header)
        };
        ...
        const addFooter = (doc, pageNum, isLastPage = false) => {
            ...
            if (isLastPage) {
                ...
                doc.text('PROBATUM SEAL v1.0-COMMERCIAL-LITIGATION',
                ...
                const _sealTexts = [
                    ...
                    'v1.0-COMMERCIAL-LITIGATION · Art. 405.º/406.º CC',
	...
	const _pdfMasterHashRaw = (UNIFEDSystem.masterHash && UNIFEDSystem.masterHash.length === 64)
       ...
       // --- FIM DA RETIFICAÇÃO ---
        ...
        doc.text(`OBJETO: RECONSTITUIÇÃO DA VERDADE MATERIAL DIGITAL / ART. 103.º Normas de Conformidade Fiscal`, left, y, { lineHeightFactor: 1.5 }); y += 4;
...

        {
            ...
            const _cedFund = doc.splitTextToSize(
                'Art. 104.º n.º 2 Normas de Conformidade Fiscal (Fraude Qualificada) e Art. 125.º CPP.',
                _cedW - 5);
            ...
            const _cedConc = doc.splitTextToSize(
                'A retenção sistemática de percentagens em comissões sem a devida faturação constitui apropriação indevida e indicia irregularidade contratual de omissão de proveitos por parte da entidade processadora.',
                _cedW - 5);
        ...
        doc.setFontSize(FONT_SUBTITULO);
        doc.setFont('helvetica', 'bold');
        doc.text('3. VEREDICTO DE RISCO / RISK VERDICT (Normas de Conformidade Fiscal · Art. 103.º)', left, y);
        y += 8;
        ...
        doc.text(`• ${currentLang === 'pt' ? 'Art. 405.º CC — Omissão de Faturação: omissão de proveitos e retenção indevida de IVA.' : 'Art. 405 CC — Tax Fraud: omission of income and improper VAT withholding.'}`, left, y, { maxWidth: doc.internal.pageSize.getWidth() - 30 }); y += 5;
        doc.text(`• ${currentLang === 'pt' ? 'Art. 406.º CC — Irregularidade Comercial Agravada: quando a omissão excede os limiares legais.' : 'Art. 406 CC — Qualified Tax Fraud: when the omission exceeds legal thresholds.'}`, left, y, { maxWidth: doc.internal.pageSize.getWidth() - 30 }); y += 8;
        ...
        if (y > 220) { y = newPage(doc); }
        {
            ...
            doc.setFontSize(FONT_SUBTITULO);
            doc.setTextColor(150, 20, 20);
            doc.text(`${currentLang === 'pt' ? 'ANÁLISE DE TIPOLOGIAS DE RISCO DETETADAS — CEJ / PJ / Normas de Conformidade Fiscal' : 'ANALYSIS OF DETECTED RISK TYPOLOGIES — CEJ / PJ / Normas de Conformidade Fiscal'}`, left + 3, y + 3);
            doc.setTextColor(0, 0, 0);
            y += 12;
            ...
            const _riskRows = [
                { tipo: currentLang === 'pt' ? 'OMISSÃO DE FATURAÇÃO' : 'TAX FRAUD',
                  fund: 'Art. 405.º CC',
                  desc: currentLang === 'pt'
                    ? 'Omissão de proveitos e retenção indevida de IVA sobre comissões. Responsabilidade Civil: Indemnização por danos.'
                    : 'Omission of income and improper VAT withholding on commissions. Penalty: imprisonment up to 3 years or fine.' },
                { tipo: currentLang === 'pt' ? 'IRREGULARIDADE COMERCIAL AGRAVADA' : 'QUALIFIED TAX FRAUD',
                  fund: 'Art. 406.º CC',
                  desc: currentLang === 'pt'
        ...
        doc.text(`Certificação: UNIFED - v1.0-COMMERCIAL-LITIGATION · ISO/IEC 27037 · D.L. n.º 28/2019`, left, y); y += 5;
        doc.setFont('helvetica', 'italic');
        doc.setFontSize(FONT_TEXTO);
        ...
        if (y > 235) { addFooter(doc, pageNumber); doc.addPage(); pageNumber++; addHeader(doc); y = PDF_MARGIN_T; }
        {
        ...
        if (y > 225) { addFooter(doc, pageNumber); doc.addPage(); pageNumber++; addHeader(doc); y = PDF_MARGIN_T; }
        {
            ...
            const _ccbLines = doc.splitTextToSize(
                currentLang === 'pt'
                    ? 'A engenharia algorítmica da plataforma cria uma \'zona cinzenta\' premeditada entre o ganho real retido na fonte ' +
                      ...
                      'uma tipologia de criminalidade de colarinho branco e discrepância de valores estruturada, explorando a assimetria de ' +
        ...
        if (y > 220) { addFooter(doc, pageNumber); doc.addPage(); pageNumber++; addHeader(doc); y = PDF_MARGIN_T; }
        {
            ...
            const _bopTech = doc.splitTextToSize(
                currentLang === 'pt'
                    ? 'Análise Técnica: A UNIFED-PROBATUM identificou uma divergência estrutural entre o Fluxo de Caixa Real ' +
                      ...
                      'encontra-se numa situação de indefesa técnica. A plataforma atua como "Opacidade Algorítmica" fiscal — o sujeito ' +
                    ...
                    : 'Technical Analysis: UNIFED-PROBATUM identified a structural divergence between the Real Cash Flow ' +
                      ...
                      'finds themselves in a situation of technical defenselessness. The platform acts as a tax "Opacidade Algorítmica" — the taxpayer ' +
            ...
            const _bopConc = doc.splitTextToSize(
                currentLang === 'pt'
                    ? 'Conclusão Pericial: Por força do Princípio da Proximidade da Prova (Acórdão STJ 11/07/2013) e do ' +
                      ...
                      'confissão implícita da apropriação indevida e da omissão de faturação aqui evidenciada. ' +
        ...
        if (_enrichSankeyImage) {
            ...
            doc.text(
                (currentLang === 'pt'
                    ? 'DIAGRAMA DE FLUXO FINANCEIRO — MONEY FLOW ANALYSIS · v1.0-COMMERCIAL-LITIGATION'
                    : 'FINANCIAL FLOW DIAGRAM — MONEY FLOW ANALYSIS · v1.0-COMMERCIAL-LITIGATION'),
            ...
            doc.setFont('courier', 'normal');
            doc.setTextColor(120, 120, 120);
            doc.text('UNIFED-PROBATUM v1.0-COMMERCIAL-LITIGATION · Diagrama de Fluxo Financeiro · Art. 125.º CPP · ISO/IEC 27037 · D.L. 28/2019', left, y + 4);
        ...
        if (_enrichTemporalImage) {
            ...
            const atfNota = doc.splitTextToSize(
                currentLang === 'pt'
                    ? 'Gráfico temporal derivado dos extratos mensais processados. ' +
                      ...
                      'constitui indício de comportamento oportunístico para efeitos do Art. 406.º CC.'
                    : 'Temporal graph derived from processed monthly statements. ' +
                      ...
                      'constitutes evidence of opportunistic behavior for the purposes of Art. 406 CC.',
                180);
            ...
            if (typeof window.computeTemporalAnalysis === 'function') {
                try {
                    ...
                    doc.setFont('courier', 'normal');
                    doc.setTextColor(120, 120, 120);
                    doc.text('UNIFED-PROBATUM v1.0-COMMERCIAL-LITIGATION · Análise Temporal Forense · ISO/IEC 27037 · D.L. 28/2019', left, y + 3);
                } catch (_e) { }
            }
            addFooter(doc, pageNumber);
        }
        ...
        if (_enrichLegalNarrative) {
            ...
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(0, 229, 255);
            doc.text(`${currentLang === 'pt' ? 'SÍNTESE JURÍDICA PERICIAL — ANÁLISE DETERMINÍSTICA v1.0-COMMERCIAL-LITIGATION' : 'FORENSIC LEGAL SUMMARY — DETERMINISTIC ANALYSIS v1.0-COMMERCIAL-LITIGATION'}`,
                doc.internal.pageSize.getWidth() / 2, 18, { align: 'center' });
            doc.setTextColor(0, 0, 0);
            y = 30;
            ...
            const aiNotaLines = doc.splitTextToSize(
                currentLang === 'pt'
                    ? 'Documento gerado sob metodologia forense UNIFED-PROBATUM v1.0-COMMERCIAL-LITIGATION. ' +
                      ...
                      'UNIFEDSystem.analysis (Fonte de Verdade Imutável) e uma base de artigos legais estática (CIVA/CIRC/CC/CPP/DAC7). ' +
                      'Conformidade: Art. 125.º CPP · ISO/IEC 27037:2012 · D.L. n.º 28/2019 de 15 de fevereiro.'
                    : 'Document generated under UNIFED-PROBATUM v1.0-COMMERCIAL-LITIGATION forensic methodology. ' +
                      ...
                      'UNIFEDSystem.analysis (Immutable Truth Source) and a static legal article base (CIVA/CIRC/CC/CPP/DAC7). ' +
            ...
            doc.text('UNIFED-PROBATUM v1.0-COMMERCIAL-LITIGATION · Análise Determinística · Base Legal: CIVA/CIRC/CC/CPP/DAC7', left, y); y += 4;
            ...
            const qLegal = doc.splitTextToSize(
                currentLang === 'pt'
                    ? '[Fundamentação Legal] Termos e Condições da Plataforma · Comissões 0% sobre gorjetas e campanhas · ' +
                      'Art. 125.º CPP (admissibilidade da prova) · Art. 405.º CC (Incumprimento Contratual) · ' +
                    ...
                    : '[Legal Basis] Platform Terms and Conditions · 0% commission on tips and campaigns · ' +
                      'Art. 125 CPP (admissibility of evidence) · Art. 405 CC (Tax Fraud) · ' +
...

        {
            ...
            if (percDiscrepancia > 15) {
                ...
                doc.setFontSize(FONT_TEXTO);
                doc.text(
                    currentLang === 'pt' ? 'Fundamento: Art. 344.º CC · Art. 74.º/75.º LGT · Art. 89.º-A LGT · Art. 405.º/406.º CC' : 'Basis: Art. 344 CC · Art. 74/75 LGT · Art. 89-A LGT · Art. 103/104 Normas de Conformidade Fiscal',
                    left, y);
                y += 7;
                doc.setFont('helvetica', 'normal');
            }
            ...
            const _cqQ1 = doc.splitTextToSize(
                currentLang === 'pt'
                    ? '"Pode a plataforma explicar a impossibilidade de reconciliação bancária direta (cruzamento 1:1) ' +
                      ...
                      'indício de ofuscação deliberada, nos termos do Art. 405.º do Código Civil."'
                    : '"Can the platform explain the impossibility of direct bank reconciliation (1:1 matching) ' +
                      ...
                      'evidence of deliberate obfuscation, under the terms of Art. 103 of the Normas de Conformidade Fiscal."',
                _cqUseW - 5);
            doc.text(_cqQ1, left + 3, y); y += (_cqQ1.length * 3.5) + 7;

            if (y > 245) { y = newPage(doc); }
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(FONT_SUBTITULO);
            doc.setTextColor(30, 60, 120);
            ...
            const _cqNota = doc.splitTextToSize(
                currentLang === 'pt'
                    ? 'Fundamentação Legal: Art. 327.º CPP (Contraditório) · Art. 125.º CPP (Admissibilidade de Prova) · ' +
                      'Art. 405.º/406.º CC (Incumprimento e Força Obrigatória) · Art. 36.º, n.º 11 CIVA · ' +
                    ...
                    : 'Legal Basis: Art. 327 CPP (Contradiction) · Art. 125 CPP (Admissibility of Evidence) · ' +
                      'Art. 103/104 Normas de Conformidade Fiscal (Tax Fraud/Qualified) · Art. 36(11) CIVA · ' +
...

        {
            ...
            const _termoTextoCont = doc.splitTextToSize(
                currentLang === 'pt'
                    ? 'constituindo Prova Digital Material inalterável para efeitos judiciais, sob égide do Art. 405.º do Código Civil, normas ISO/IEC 27037 e Decreto-Lei n.º 28/2019.'
                    : 'constituting unalterable Material Digital Evidence for judicial purposes, under the aegis of Art. 405 CC, ISO/IEC 27037 standards and Decree-Law No. 28/2019.',
                _termUW);
            ...
            const _cpp125Lines = doc.splitTextToSize(
                currentLang === 'pt'
                    ? 'São admissíveis como meios de prova todos os meios não proibidos por lei (Art. 125.º do Código de Processo Penal Português). ' +
                      ...
                      'A omissão de IVA apurada fundamenta a qualificação do facto nos termos dos Art. 405.º CC (Incumprimento Contratual) e Art. 406.º CC (Incumprimento Agravado) do Normas de Conformidade Fiscal.'
                    : 'All means of proof not prohibited by law are admissible (Art. 125 of the Portuguese Code of Criminal Procedure). ' +
                      ...
                      'The missing VAT found supports the qualification of the fact under the terms of Art. 103 (Tax Fraud) and Art. 104 (Qualified Tax Fraud) of the Normas de Conformidade Fiscal.',
                _termUW);
            ...
            doc.setFontSize(FONT_SUBTITULO);
            doc.setTextColor(30, 60, 120);
            doc.text('[ UNIFED - PROBATUM CERTIFIED · ANALISTA E CONSULTOR FORENSE · v1.0-COMMERCIAL-LITIGATION ]',
            ...
            doc.setFontSize(FONT_TEXTO);
            doc.text(`${currentLang === 'pt' ? 'Fundamentação: Normas de Conformidade Fiscal Art. 405.º CC (Incumprimento Contratual) · Art. 104.º (Fraude Qualificada) · CRP Art. 32.º · CPP Art. 125.º' : 'Basis: Normas de Conformidade Fiscal Art. 103 (Tax Fraud) · Art. 104 (Qualified Fraud) · CRP Art. 32 · CPP Art. 125'}`,
	 ...
	 // --- RETIFICAÇÃO UNIFED-SCRIPT-RET-08 ---
        ...
        // PATCH 2 — Resolução da Race Condition da Árvore Merkle:
        // Invocar explicitamente e AGUARDAR a resolução do Merkle Root ANTES
        // da serialização do PDF. Elimina setTimeout arbitrário de 520ms.
        if (typeof applyTimestampAndMerkle === 'function') {
            await applyTimestampAndMerkle();
        }

        // Extrair a Raiz Merkle consolidada para uso no PDF e na cadeia de custódia.
        const raizMerklePDF = UNIFEDSystem.analysis.merkleRoot
            || window.UNIFED_FORENSIC_SYSTEM?.chainOfCustody?.masterHash
            || UNIFEDSystem.masterHash
            || 'N/A';

        // Emitir evento secundário para sincronização de outros módulos (listeners em script_injection.js).
